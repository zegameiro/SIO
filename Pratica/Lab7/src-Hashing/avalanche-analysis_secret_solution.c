...
// gcc -O2 -Wall sha256.c md5.c avalanche-analysis_secret_solution.c -o avalanche-analysis_secret_solution
...
// TODO similar to changeMeABit but a byte!
    ...
    const uint8_t bmask = 0xFF;                // a mask with all bits set

    uint32_t try=0;
    while(track_mods[idx]) { // while we hit in a byte that was already changed
        idx=(idx+1)%source_size;
        if(try++==source_size) { // if all bytes were already modified
            // TODO: avoid it as in `changeMeABit(...)`!!
            iWouldPreferNotToContinue(__LINE__);
        }
    }

    working_buf[idx]^=(bmask); // change the all byte
    track_mods[idx]  = 1;      // stores the action

    if(8!=countDiffBits(source,working_buf,source_size)) {
        iWouldPreferNotToContinue(__LINE__);
    }
}
 ...
 */
     ...
     * OPTION:
     * use md5 or sha256
     * (!!name is not shown in usage!!)
     *********************************************/
    const int size_of_hash = BYTES_NEED_FOR_SHA256;
    sha256(source, source_size, source_hash);

    //const int size_of_hash = BYTES_NEED_FOR_MD5;
    //md5(source, source_size, source_hash);
    ...
    **********************************************/
    ...
    for(int i=0;i<n_variations;i++) {
        ...
        /**
         * OPTION:
         * change a bit or a byte
         * (!!not considered in usage!!)
         ********************************************************/

        //changeMeAByte(source, source_size, track_mods, bit_mod);
