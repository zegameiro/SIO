...
app = Flask(__name__)
app.secret_key = 'sio_first_project'

mysql = MySQL()
app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PASSWORD'] = 'password' # NOTE: Obviously unsafe :)
...
def login():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if result > 0:
            ...
            # Unsafe
            if hashlib.md5(password_cand.encode()).hexdigest() == password:
...
def shoppingcart():
    ...
    if request.method == 'POST': # Confirm order containing all shopping items and their respective quantities
        ...
        c.execute("DELETE FROM Cart WHERE usrID = '" + usrID + "'")
    ...
    if 'user' in request.args:
        ...
        if result:
            ...
            if request.method == 'POST':
                ...
                # Create Cursor
                cur = mysql.connection.cursor()
                exe = cur.execute("UPDATE UserP SET first_name=%s, last_name=%s, username=%s, email=%s, passwd=%s, descript=%s, phoneNum=%s WHERE usrID=%s",
                                    (first_name, last_name, username, email, password, description, mobile, result['usrID']))
                if exe:
                    mysql.connection.commit()
    ...
    if "is_admin_logged_in" in session:
        if session['is_admin_logged_in'] == True:
            if request.method == 'POST':
                ...
                curs.execute("SELECT catID FROM Category WHERE nome = '" + category + "'")
                ...
                # Most unsafe, allows any file and doesn't convert
